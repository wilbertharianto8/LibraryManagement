...
<html>
    ...
    <body>
        ...
        <sql:query dataSource = "${snapshot}" var = "result">
            SELECT * from staff WHERE username = '<%= session.getAttribute("username")%>';
        </sql:query>

        <c:forEach var = "row" items = "${result.rows}">
            <c:set var = "staffId" value = "${row.staffId}" />
            <c:set var = "staffUsername" value = "${row.username}" />
            <c:set var = "staffFullName" value = "${row.fullname}" />  
            <c:set var = "staffEmail" value = "${row.email}" />
            <c:set var = "staffAddress" value = "${row.address}" />
            <c:set var = "staffPassword" value = "${row.password}" />
            <c:set var = "staffPhone" value = "${row.phone}" />    
        </c:forEach>
            ...
            <div class="profileText d-table">
                <div class="d-table-cell align-middle">                    
                    <div class="col d-flex flex-column">
                        <div class="h4 font-weight-bold">${staffFullName}</div>
                        <div class="h6"><span class="emailText">${staffEmail}</span> - <span class="userTypeText">Staff</span></div>
                    </div>
                </div>
            </div>            
        </div>                    
        <div class="container d-flex flex-column mt-5">
            <form method="post" action="updateProfileServlet">
                ...
                <div class="w-75 d-flex flex-row mt-4">
                    ...
                    <input name="username" class="w-75" type="text" value="${staffUsername}" />                
                </div>
                <div class="w-75 d-flex flex-row mt-4">
                    <div class="h6 w-25 mt-1 fieldName">Full Name</div>
                    <input name="fullname" class="w-75" type="text" value="${staffFullName}" />                
                </div>
                <div class="w-75 d-flex flex-row mt-4">
                    <div class="h6 w-25 mt-1 fieldName">Email</div>
                    <input name="email" class="w-75" type="email" value="${staffEmail}" />                
                </div>
                <div class="w-75 d-flex flex-row mt-4">
                    <div class="h6 w-25 mt-1 fieldName">Address</div>
                    <input name="address" class="w-75" type="text" value="${staffAddress}" />                
                </div>
                <div class="w-75 d-flex flex-row mt-4 mb-3">
                    <div class="h6 w-25 mt-1 fieldName">Phone Number</div>
                    <input name="phone" class="w-75" type="phone" value="${staffPhone}" />                
                </div> 
